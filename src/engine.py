...
def _call_tool(name: str, args: Dict[str, Any], state: Dict[str, Any]):(name: str, args: Dict[str, Any], state: Dict[str, Any]):
...
def _maybe_handle_deterministic(query: str, transactions):
    ...
    # rolling-window interest
    n = parse_last_n_months(q)
    if n and 'interest' in q:
        total, per_acct, ids = _sum_interest_last_n_months(transactions, n)
        return {
            "answer": json.dumps({"total_interest": total, "per_account": per_acct}),
            "reasoning": f"Summed INTEREST over last {n} months (latest based on data)",
            "sources": ids[:25]
        }
    ...
    if n and ('statement' in q or 'summary' in q):
        ...
        return {
            "answer": json.dumps({"months": stmt}),
            "reasoning": f"Computed inflow/outflow/net for last {n} months",
            "sources": ids[:25]
        }
...
def _months_in_range(transactions, last_n: int):
    # Determine anchor: latest transaction date (UTC ISO), else today
    latest = None
...
def _sum_interest_last_n_months(transactions, last_n: int):
    ...
    for t in transactions:
        if (t.transaction_type or '').upper() == 'INTEREST' and month_key(t.transaction_date_time) in months:
            ...
            per_account.setdefault(acct, 0.0)
            per_account[acct] += (t.amount or 0.0)
    # round
...
def _statement_summary_last_n_months(transactions, last_n: int):
    months = _months_in_range(transactions, last_n)
    # Summarize inflow/outflow/net and counts
    summary = {}
    ids = []
    for t in transactions:
